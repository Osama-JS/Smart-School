...
<head>
    <meta charset="UTF-8">
    <title>{{ $printSettings['title'] ?? 'تقرير إجازات الموظفين' }}</title>
...
<body class="p-4 sm:p-8">
            ...
            <div class="flex flex-col gap-0.5 z-10 w-[38%] pt-0.5">
                ...
                <div class="mt-3 pr-3 border-r-[3px] py-0.5 brand-border">
                    <h1 class="text-xl font-black text-slate-900 tracking-tight whitespace-nowrap leading-none">
                        {{ $printSettings['title'] ?? 'تقرير إجازات الموظفين' }}
                    </h1>
                </div>
            </div>
    ...
    <div class="grid grid-cols-3 gap-3 mb-8">
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">إجمالي طلبات الإجازات</p>
                <p class="text-lg font-black leading-none text-slate-800">{{ $kpis['total_leaves'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">إجمالي الأيام الممنوحة</p>
                <p class="text-lg font-black leading-none text-emerald-600">{{ $kpis['total_leave_days'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">موظفين مجازين</p>
                <p class="text-lg font-black leading-none text-primary-600">{{ $kpis['unique_employees_on_leave'] ?? 0 }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <table class="w-full text-right border-collapse border border-slate-200 mt-2">
        <thead>
            <tr class="brand-bg">
                ...
                <th class="border border-slate-200 p-2 font-bold text-center">نوع الإجازة</th>
                <th class="border border-slate-200 p-2 font-bold text-center">تاريخ الإجازة</th>
                <th class="border border-slate-200 p-2 font-bold text-center">الأيام</th>
            ...
            @forelse($employeesData as $employee)
                @foreach($employee['records'] as $record)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="border border-slate-200 p-2 text-center font-bold text-emerald-600">{{ $record['days'] }}</td>
                        <td class="border border-slate-200 p-2 text-center text-xs text-slate-500 max-w-[150px] truncate" title="{{ $record['reason'] }}">{{ $record['reason'] ?: '-' }}</td>
                    </tr>
                @endforeach
            @empty
                <tr>
