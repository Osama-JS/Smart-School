...
<head>
    <meta charset="UTF-8">
    <title>{{ $printSettings['title'] ?? 'تقرير الخطط الدراسية وانضباط التسليم' }}</title>
...
<body class="p-4 sm:p-8">
            ...
            <div class="flex flex-col gap-0.5 z-10 w-[38%] pt-0.5">
                ...
                <div class="mt-3 pr-3 border-r-[3px] py-0.5 brand-border">
                    <h1 class="text-xl font-black text-slate-900 tracking-tight whitespace-nowrap leading-none">
                        {{ $printSettings['title'] ?? 'تقرير الخطط الدراسية وانضباط التسليم' }}
                    </h1>
                </div>
            </div>
    ...
    <div class="grid grid-cols-4 gap-3 mb-8">
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">إجمالي الخطط</p>
                <p class="text-lg font-black leading-none text-slate-800">{{ $kpis['total_plans'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">الخطط المعتمدة</p>
                <p class="text-lg font-black leading-none text-emerald-600">{{ $kpis['total_approved'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">قيد المراجعة</p>
                <p class="text-lg font-black leading-none text-amber-500">{{ $kpis['total_pending'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div class="overflow-hidden">
                <p class="text-slate-500 text-[11px] font-bold mb-0.5 truncate">لم تسلم / مرفوضة</p>
                <p class="text-lg font-black leading-none text-rose-600">{{ ($kpis['total_missing'] ?? 0) + ($kpis['total_rejected'] ?? 0) }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <table class="w-full text-right border-collapse border border-slate-200 mt-2">
        <thead>
            <tr class="brand-bg">
                ...
                <th class="border border-slate-200 p-2 font-bold text-center">إجمالي الخطط</th>
                <th class="border border-slate-200 p-2 font-bold text-center">معتمدة</th>
                <th class="border border-slate-200 p-2 font-bold text-center">قيد المراجعة</th>
                <th class="border border-slate-200 p-2 font-bold text-center">مرفوضة / مسودة</th>
                <th class="border border-slate-200 p-2 font-bold text-center">لم تُقدم</th>
            </tr>
        </thead>
        <tbody>
            @php $count = 1; @endphp
            @forelse($teachersData as $teacher)
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="border border-slate-200 p-2 text-center font-bold text-slate-700">{{ $teacher['total_plans'] }}</td>
                    <td class="border border-slate-200 p-2 text-center font-bold text-emerald-600">{{ $teacher['approved_plans'] }}</td>
                    <td class="border border-slate-200 p-2 text-center font-bold text-amber-600">{{ $teacher['pending_plans'] }}</td>
                    <td class="border border-slate-200 p-2 text-center font-bold text-rose-500">{{ $teacher['rejected_plans'] + $teacher['draft_plans'] }}</td>
                    <td class="border border-slate-200 p-2 text-center font-bold text-rose-700">{{ $teacher['missing'] }}</td>
                </tr>
            @empty
                <tr>
