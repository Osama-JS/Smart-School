...
export default function AdministrativeRequests({ employeesData, kpis, departmentChartData, allEmployees, departments, periodStart, periodEnd, filters }) {
    ...
    const [printSettings, setPrintSettings] = useState(() => {
        ...
        try {
            const saved = localStorage.getItem('AdministrativeRequestsPrintSettings');
    ...
    useEffect(() => {
        localStorage.setItem('AdministrativeRequestsPrintSettings', JSON.stringify(printSettings));
    }, [printSettings]);// KPIs Calculations (from backend)
    const { total_requests = 0, unique_employees_with_requests = 0 } = kpis || {};
    ...
    if (requestsOnly) activeFilters.push({ id: 'requests', label: `مقدمي الطلبات فقط` });
    ...
    const customStyles = {
        control: (provided, state) => ({
            ...
            borderColor: state.isFocused ? '#10b981' : '#e2e8f0', // using emerald-500 as primary
    ...
    return (
        <AdminLayout activeMenu="التقارير">
            <Head title="تقرير الطلبات الإدارية" />
                        ...
                        <div className="overflow-hidden">
                            <form onSubmit={handleFilter} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                                ...
                                <div className="xl:col-span-4 flex flex-col xl:flex-row justify-between items-center gap-4 mt-2">
                                    ...
                                    <div 
                                        className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 cursor-pointer w-full xl:w-auto ${requestsOnly ? 'bg-emerald-50/50 border-emerald-200 shadow-sm' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`}
                                        onClick={() => setRequestsOnly(!requestsOnly)}
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={`relative w-12 h-6 rounded-full transition-colors duration-300 ${requestsOnly ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                                                <div className={`absolute top-1 bg-white w-4 h-4 rounded-full shadow-sm transition-all duration-300 ${requestsOnly ? 'left-1' : 'right-1'}`}></div>
                                            </div>
                                            <div>
                                                <div className={`font-bold text-[15px] leading-none mb-1 ${requestsOnly ? 'text-emerald-700' : 'text-slate-700'}`}>عرض مقدمي الطلبات فقط</div>
                                                <div className={`text-xs font-semibold ${requestsOnly ? 'text-emerald-500' : 'text-slate-500'}`}>من قدم طلبات</div>
                                            </div>
                                        </div>
                                    </div>
        ...
        <ReportPrintLayout title={printSettings.title} printSettings={printSettings} setPrintSettings={setPrintSettings} onPrint={handlePrintClick} onDownloadPdf={handleDownloadPDF} isGeneratingPdf={isGeneratingPdf} startDate={startDate} endDate={endDate}>
                    ...
                    {printSettings.showKPIs && sortedEmployeesData.length > 0 && (
                        <div className="grid grid-cols-4 gap-3 mb-8">
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي الطلبات</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{total_requests}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">طلبات غير مكتملة</p>
                                    <p className="text-lg font-black leading-none text-slate-800">-</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">مقدمو الطلبات</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{unique_employees_with_requests}</p>
                                </div>
                            </div>
                            ...
                            <div className="h-[240px] w-full" dir="ltr">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                                        ...
                                        <Bar dataKey="total_requests" name="إجمالي الطلبات" fill="#f59e0b" radius={[4, 4, 0, 0]} barSize={28} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                    ...
                    {sortedEmployeesData.length > 0 ? (
                        ...
                        <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-100 border-dashed print:hidden">
                            ...
                            <h3 className="text-lg font-bold text-slate-800 mb-1">لا توجد سجلات طلبات</h3>
                            <p className="text-slate-500">لم يتم العثور على أي طلبات تطابق الفلاتر المحددة.</p>
                        </div>
                    )}
...
const EmployeeRowGroup = ({ employeeName, data, printSettings }) => {
    ...
    const requestsCount = recordsArray.length;
    const isHighAchiever = requestsCount >= 5;
        ...
        <React.Fragment>
                        ...
                        <div className="flex gap-2 items-center">
                            {requestsCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/10 print:bg-transparent print:ring-amber-700">
                                    ...
                                    <span>عدد الطلبات: {requestsCount}</span>
                                </div>
                            )}
                            {requestsCount === 0 && (
            ...
            {recordsArray.map((req, index) => (
                ...
                >
                    ...
                    <td className="py-3 px-4 text-slate-700 font-bold">{req.type_name}</td>
                    <td className="py-3 px-4 text-center">
                        <span className={`font-semibold px-2 py-1 rounded-full text-xs ${req.status === 'معتمد' ? 'bg-emerald-100 text-emerald-700' : req.status === 'مرفوض' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{req.status ? req.status : 'معلق'}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                        <span className="font-semibold text-slate-600 text-sm">{req.details ? req.details : '-'}</span>
                    </td>
                </tr>
            ))}
        </React.Fragment>
