...
import { ArrowRight, Save, User, Lock, Phone, Mail, MapPin, Eye, EyeOff, ShieldAlert, Users, Bus, GraduationCap, Building2, Plus, X, AtSign } from 'lucide-react';
    ...
    return (
        <AdminLayout activeMenu="الطلاب">
            ...
            <Modal show={isAddParentModalOpen} onClose={() => setIsAddParentModalOpen(false)} maxWidth="md">
                <div className="bg-white dark:bg-[#121820] rounded-[2rem] overflow-hidden">
                    <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
                        ...
                        <button type="button" onClick={() => setIsAddParentModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all">
                            <X size={20} strokeWidth={2.5} />
                        </button>
                    </div>
                    
                    <form onSubmit={handleQuickAddParent} className="p-6 space-y-5">
                        <div className="space-y-2">
                            ...
                            <input type="text" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 py-3.5 text-base outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all font-bold"
                                value={parentForm.name} onChange={e => setParentForm({...parentForm, name: e.target.value})} required placeholder="مثال: أحمد محمد" />
                            {parentFormErrors.name && <p className="text-xs font-bold text-rose-500 mt-1">{parentFormErrors.name[0]}</p>}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <div className="space-y-2">
                                ...
                                <input type="text" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 py-3.5 text-base outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all font-bold"
                                    value={parentForm.username} onChange={e => setParentForm({...parentForm, username: e.target.value})} dir="ltr" required placeholder="مثال: ahmed123" />
                                {parentFormErrors.username && <p className="text-xs font-bold text-rose-500 mt-1">{parentFormErrors.username[0]}</p>}
                            </div>
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 text-sm font-black text-slate-700 dark:text-slate-300">
                                ...
                                <input type="password" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 py-3.5 text-base outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all font-bold"
                                    value={parentForm.password} onChange={e => setParentForm({...parentForm, password: e.target.value})} dir="ltr" required placeholder="********" />
                                {parentFormErrors.password && <p className="text-xs font-bold text-rose-500 mt-1">{parentFormErrors.password[0]}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <div className="space-y-2">
                                ...
                                <input type="text" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 py-3.5 text-base outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all font-bold"
                                    value={parentForm.national_id} onChange={e => setParentForm({...parentForm, national_id: e.target.value})} dir="ltr" placeholder="10XXXXXXXX" />
                            </div>
                            <div className="space-y-2">
                                <label className="flex items-center gap-2 text-sm font-black text-slate-700 dark:text-slate-300">
                                ...
                                <input type="text" className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl px-5 py-3.5 text-base outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all font-bold"
                                    value={parentForm.phone} onChange={e => setParentForm({...parentForm, phone: e.target.value})} dir="ltr" placeholder="05XXXXXXXX" />
                            </div>
                        </div>
                        
                        <div className="flex gap-4 pt-4 border-t border-slate-100 dark:border-slate-800 mt-6">
                            <button type="submit" disabled={isSubmittingParent} className="flex-[2] py-3.5 text-sm font-black flex justify-center items-center gap-2 text-white bg-gradient-to-r from-primary-500 to-primary-600 hover:from-primary-600 hover:to-primary-700 rounded-2xl disabled:opacity-70 transition-all shadow-lg shadow-primary-500/30">
                                {isSubmittingParent ? (
                                    <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                ) : (
                                    <><Save size={18} /> حفظ وإضافة</>
                                )}
                            </button>
                            <button type="button" onClick={() => setIsAddParentModalOpen(false)} className="flex-1 flex justify-center items-center gap-2 py-3.5 text-sm font-black text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                <X size={18} />
                                إلغاء
                            </button>
                        </div>
