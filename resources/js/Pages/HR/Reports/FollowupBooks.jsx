...
import { 
    ...
    BookOpen, Settings, Download, Eye, AlertCircle, FileText, Check 
} from 'lucide-react';
...
export default function FollowupBooksReport({ 
    auth, 
    teachers = [], 
    kpis = {}, 
    departmentChartData = [], 
    ...
    const statusOptions = [
        { value: 'published', label: 'تم التحضير (في الوقت)' },
        { value: 'late', label: 'تحضير متأخر' },
        { value: 'draft', label: 'مسودة' },
        { value: 'missing', label: 'لم يتم التحضير (تقصير)' }
    ];

    const initialStatuses = filters.statuses 
        ? (Array.isArray(filters.statuses) ? filters.statuses : filters.statuses.split(',')).map(v => statusOptions.find(o => o.value === v)).filter(Boolean)
        : statusOptions;
    ...
    // Form for settings modal
    const { data: settingsData, setData: setSettingsData, post: postSettings, processing: settingsProcessing, errors: settingsErrors, reset: resetSettings } = useForm({
    ...
    const [printSettings, setPrintSettings] = useState(() => {
        const defaultSettings = {
            title: 'تقرير رفع دفتر المتابعة وانضباط التحضير',
            showKPIs: true,
            showDetails: true,
        ...
        try {
            const saved = localStorage.getItem('followupBooksPrintSettings');
    ...
    useEffect(() => {
        localStorage.setItem('followupBooksPrintSettings', JSON.stringify(printSettings));
    }, [printSettings]);
    ...
    const totalExpected = kpis?.total_expected || 0;
    const totalPublished = kpis?.total_published || 0;
    const totalLate = kpis?.total_late || 0;
    const totalDraft = kpis?.total_draft || 0;
    const totalNegligence = kpis?.total_negligence || 0;
    const uniqueTeachers = kpis?.unique_teachers || 0;

    const chartData = safeChartData || [];

    let sortedTeachers = [...safeTeachers].sort((a, b) => (b.negligence + b.late_uploads) - (a.negligence + a.late_uploads));

    if (violatorsOnly) {
        sortedTeachers = sortedTeachers.filter(t => t.negligence > 0 || t.late_uploads > 0);
    }

    const getTargetRoute = () => {
        try {
            return route('hr.reports.followup-books');
        } catch (e) {
            return route('admin.followup-books.index');
        }
    };

    const handleFilter = (e) => {
        if(e) e.preventDefault();
        router.get(getTargetRoute(), {
    ...
    const removeFilter = (filterId) => {
        ...
        if (filterId === 'status') { newStatuses = statusOptions; setSelectedStatuses(statusOptions); }
    ...
    if (selectedTeacher) activeFilters.push({ id: 'teacher', label: `المعلم: ${selectedTeacher.label}` });
    if (selectedStatuses && selectedStatuses.length > 0 && selectedStatuses.length < statusOptions.length) {
        activeFilters.push({ id: 'status', label: `الحالات: ${selectedStatuses.map(s => s.label).join(', ')}` });
    }
    if (violatorsOnly) activeFilters.push({ id: 'violators', label: `المتقاعسين / غير المنضبطين فقط` });
    ...
    const closeSettingsModal = () => { setIsSettingsModalOpen(false); resetSettings(); };

    const submitSettings = (e) => {
        e.preventDefault();
        postSettings(route('admin.followup-books.settings'), {
            onSuccess: () => {
                closeSettingsModal();
                Swal.fire({
                    icon: 'success',
                    title: 'تم الحفظ',
                    text: 'تم حفظ مواعيد رفع التحضير بنجاح',
    ...
    return (
        <AdminLayout user={auth?.user} activeMenu="التقارير">
            <Head title="تقرير رفع دفتر المتابعة وانضباط التحضير" />
                ...
                <div className="print:hidden relative overflow-hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        ...
                        <div>
                            <h1 className="text-[22px] font-black text-slate-800 tracking-tight mb-1">
                                تقرير رفع دفتر المتابعة وانضباط التحضير
                            </h1>
                            <p className="text-[13.5px] font-bold text-slate-500">متابعة إعداد ونشر دفاتر التحضير والمتابعة ونسب إنجاز المعلمين</p>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-3 relative z-10 w-full sm:w-auto">
                        ...
                        <button
                            ...
                            className="flex items-center justify-center gap-2 px-5 py-3 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 transition-all font-bold border border-slate-200"
                        >
                            ...
                            <div className="overflow-hidden">
                                <form onSubmit={handleFilter} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                                    ...
                                    <div className="xl:col-span-2">
                                        <label className="block text-sm font-semibold text-slate-700 mb-1.5">حالة التحضير</label>
                                        <Select
                                            isMulti
                                    ...
                                    <div className="xl:col-span-4 flex flex-col xl:flex-row justify-between items-center gap-4 mt-2">
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <div className={`font-bold text-[15px] leading-none mb-1 ${violatorsOnly ? 'text-rose-700' : 'text-slate-700'}`}>عرض المتقاعسين / غير المنضبطين فقط</div>
                                                    <div className={`text-xs font-semibold ${violatorsOnly ? 'text-rose-500' : 'text-slate-500'}`}>من لديهم عدم تحضير أو تأخير في التقديم</div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        >
                            ...
                            <div className="relative mb-5 pb-4 border-b-2" style={{ borderColor: printSettings.brandColor }}>
                                ...
                                <div className="flex justify-between items-start pt-4">
                                    ...
                                    <div className="flex flex-col gap-0.5 z-10 w-[38%] pt-0.5">
                                        ...
                                        <h3 className="text-[11px] font-semibold text-slate-500 pr-3.5 flex items-center gap-1.5 mt-0.5">المركز الرئيسي <span className="w-1 h-1 rounded-full bg-slate-300"></span> الشؤون الأكاديمية والمتابعة</h3>
                                        ...
                                        <div className="w-full max-w-[230px] bg-slate-50/50 print:bg-transparent border border-slate-200 print:border-slate-300 rounded-lg overflow-hidden shadow-sm print:shadow-none">
                                            ...
                                            <div className="p-3 flex flex-col gap-2.5">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span className="font-black text-slate-800 text-[11px]" dir="ltr">FB-{new Date().getFullYear().toString().substr(-2)}0{Math.floor(Math.random() * 900) + 100}</span>
                                                </div>
                                                ...
                                                <div className="flex justify-between items-center">
                                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">حد الرفع اليومي</span>
                                                    <span className="font-bold text-slate-800 text-[11px] font-mono tracking-wide" dir="ltr">{timeLimit}</span>
                                                </div>
                            ...
                            {printSettings.showKPIs && sortedTeachers.length > 0 && (
                                <div className="grid grid-cols-4 gap-3 mb-8">
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي الحصص</p>
                                            <p className="text-lg font-black leading-none text-slate-800">{totalExpected}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">التحضير المنشور</p>
                                            <p className="text-lg font-black leading-none text-emerald-700">{totalPublished}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">التحضير المتأخر</p>
                                            <p className="text-lg font-black leading-none text-amber-700">{totalLate}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        <div className="absolute top-0 right-0 bottom-0 w-1 opacity-80" style={{ backgroundColor: printSettings.brandColor }}></div>
                                        <div className="w-9 h-9 rounded-lg flex items-center justify-center mr-1 bg-rose-50 text-rose-600">
                                            <AlertTriangle size={18} />
                                        </div>
                                        <div className="overflow-hidden">
                                            <p className="text-slate-500 text-xs font-bold mb-0.5 truncate">غير محضر (التقصير)</p>
                                            <p className="text-lg font-black leading-none text-rose-700">{totalNegligence}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {printSettings.showKPIs && chartData.length > 0 && (
                                <div className="mt-4 bg-white border border-slate-200 rounded-xl p-5 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden mb-6">
                                    ...
                                    <h4 className="text-[15px] font-black text-slate-800 mb-5 flex items-center gap-2">
                                        إحصائيات انضباط التحضير حسب الأقسام
                                    </h4>
                                    <div className="h-[240px] w-full" dir="ltr">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                                                ...
                                                <Bar dataKey="published" name="تم التحضير" stackId="a" fill="#10b981" radius={[0, 0, 4, 4]} barSize={28} />
                                                <Bar dataKey="late" name="متأخر" stackId="a" fill="#f59e0b" radius={[0, 0, 0, 0]} />
                                                <Bar dataKey="draft" name="مسودة" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
                                                <Bar dataKey="missing" name="غير محضر" stackId="a" fill="#ef4444" radius={[4, 4, 0, 0]} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
                            ...
                            {sortedTeachers.length > 0 ? (
                                    ...
                                    <table className="w-full text-right border-collapse text-sm">
                                        <thead className={`${printSettings.ecoMode ? 'bg-slate-100 text-slate-800 border-b-2 border-slate-800' : 'text-white'}`} style={!printSettings.ecoMode ? { backgroundColor: printSettings.brandColor } : {}}>
                                            <tr>
                                                ...
                                                <th className="py-3 px-4 font-bold border-l border-white/20">اليوم</th>
                                                <th className="py-3 px-4 font-bold text-center w-32 border-l border-white/20">التاريخ</th>
                                                <th className="py-3 px-4 font-bold border-l border-white/20">المادة والصف</th>
                                                <th className="py-3 px-4 font-bold border-l border-white/20">عنوان الدرس</th>
                                                <th className="py-3 px-4 font-bold text-center w-32 border-l border-white/20 last:rounded-tl-xl print:last:rounded-none">الحالة ووقت الرفع</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                ...
                                <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-100 border-dashed print:hidden">
                                    ...
                                    <h3 className="text-lg font-bold text-slate-800 mb-1">لا توجد بيانات تحضير</h3>
                                    <p className="text-slate-500">لم يتم العثور على أي سجلات تحضير تطابق الفلاتر المحددة.</p>
                                </div>
                            )}
                            ...
                            <div className="mt-auto pt-16 relative">
                                ...
                                <div className="grid grid-cols-3 gap-6 mb-8 text-center print:grid">
                                    <div>
                                        <p className="text-xs font-bold text-slate-500 mb-12">مُعد التقرير (مسؤول المتابعة)</p>
                                        <p className="text-sm font-black text-slate-800">....................................</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-bold text-slate-500 mb-12">المشرف الأكاديمي / رئيس القسم</p>
                                        <p className="text-sm font-black text-slate-800">....................................</p>
                                    </div>
                                    <div>
            ...
            <Modal show={isSettingsModalOpen} onClose={closeSettingsModal} maxWidth="md">
                <div className="p-6">
                    ...
                    <form onSubmit={submitSettings} className="space-y-4">
                        <div>
                            <InputLabel htmlFor="time_limit" value="أقصى وقت يومي لرفع التحضير (الموعد النهائي)" />
                            <div className="mt-1">
                                <FlatpickrInput
                                    id="time_limit"
                                    value={settingsData.time_limit}
                                    onChange={(date, dateStr) => setSettingsData('time_limit', dateStr)}
                                    options={{
                                        enableTime: true,
                                        noCalendar: true,
                                        dateFormat: "H:i",
                            ...
                            <p className="text-xs text-slate-500 mt-1">يُعتبر التحضير المرفوع بعد هذا الوقت في يوم الدرس "تحضيراً متأخراً".</p>
                            <InputError message={settingsErrors.time_limit} className="mt-2" />
                        </div>
                        ...
                        <div className="mt-6 flex justify-end gap-3">
                            ...
                            <button
                                ...
                                className="px-5 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700 font-bold flex items-center gap-2"
                            >
                                {settingsProcessing && <Loader2 size={16} className="animate-spin" />}
                                حفظ التغييرات
                            </button>
                        </div>
                    </form>
...
const TeacherRowGroup = ({ teacher, printSettings }) => {
    const [isExpanded, setIsExpanded] = useState(true);
    const recordsArray = Array.isArray(teacher?.records) ? teacher.records : (teacher?.records ? Object.values(teacher.records) : []);
    const publishedCount = teacher.published_preparations || 0;
    const lateCount = teacher.late_uploads || 0;
    const draftCount = teacher.draft_preparations || 0;
    const missingCount = teacher.negligence || 0;
    
    const isWarning = missingCount > 0 || lateCount > 0;
        ...
        <React.Fragment>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className={`text-xs font-medium mt-0.5 ${isWarning ? 'text-amber-700/80' : 'text-slate-500'}`}>
                                        قسم {teacher.department} • إجمالي الحصص المستهدفة: {teacher.total_weekly_lessons || recordsArray.length}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2 items-center">
                            {publishedCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 print:bg-transparent print:ring-emerald-700">
                                    ...
                                    <span>تم التحضير: {publishedCount}</span>
                                </div>
                            )}
                            {lateCount > 0 && (
                            ...
                            {draftCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20 print:bg-transparent print:ring-blue-700">
                                    <FileText size={14} className="print:hidden text-blue-500" />
                                    <span>مسودة: {draftCount}</span>
                                </div>
                            )}
                            {missingCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20 print:bg-transparent print:ring-rose-700">
                                    <XCircle size={14} className="print:hidden text-rose-500" />
                                    <span>تقصير: {missingCount}</span>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {recordsArray.map((record, index) => (
                ...
                >
                    ...
                    <td className="py-3 px-4 text-slate-700 font-bold">{record.day}</td>
                    <td className="py-3 px-4 text-center" dir="ltr">
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-slate-600 font-mono text-xs tracking-wider shadow-sm print:shadow-none print:bg-transparent print:border-none print:p-0">
                            ...
                            <span>{formatDateStr(record.date)}</span>
                        </div>
                    </td>
                    <td className="py-3 px-4 text-slate-800 font-semibold">
                        {record.subject_name} {record.grade_name ? `(${record.grade_name} ${record.division_name ? '- ' + record.division_name : ''})` : ''}
                    </td>
                    <td className="py-3 px-4 text-slate-600">{record.lesson_title || 'غير محدد'}</td>
                    <td className="py-3 px-4 text-center">
                        {record.status_code === 'published' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 print:bg-transparent print:ring-emerald-700">
                                <CheckCircle size={12} className="print:hidden"/> {record.status} ({record.created_at})
                            </span>
                        )}
                        {record.status_code === 'late' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700">
                                <Clock size={12} className="print:hidden"/> {record.status} ({record.created_at})
                            </span>
                        )}
                        {record.status_code === 'draft' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20 print:bg-transparent print:ring-blue-700">
                                <FileText size={12} className="print:hidden"/> {record.status}
                            </span>
                        )}
                        {record.status_code === 'missing' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20 print:bg-transparent print:ring-rose-700">
                                <XCircle size={12} className="print:hidden"/> {record.status}
                            </span>
                        )}
                    </td>
                </tr>
