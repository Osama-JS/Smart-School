...
export default function AcademicStructureIndex({ academicYears, selectedYearId, sections, divisions, teachers, isAdmin, branches = [] }) {
    ...
    const submitGrade = (e) => {
        ...
        if (editingItem) {
            ...
            router.post(route('academic.grades.store'), gradeForm, { onSuccess: () => setIsGradeModalOpen(false) });
        }
    };
            ...
            <Modal isOpen={isGradeModalOpen} onClose={() => setIsGradeModalOpen(false)} title={editingItem ? "تعديل صف" : "إضافة صف جديد"}>
                <form onSubmit={submitGrade} className="space-y-4">
                    <div>
                        ...
                        <input type="text" value={gradeForm.name} onChange={e => setGradeForm({...gradeForm, name: e.target.value})} placeholder="مثال: الأول الابتدائي" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all dark:bg-slate-800 dark:border-slate-700 dark:text-white" required />
                    </div>
                    <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
