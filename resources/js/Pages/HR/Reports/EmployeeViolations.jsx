...
export default function EmployeeViolations({ employeesData, kpis, departmentChartData, allEmployees, departments, periodStart, periodEnd, filters }) {
    ...
    const [printSettings, setPrintSettings] = useState(() => {
        ...
        try {
            const saved = localStorage.getItem('EmployeeViolationsPrintSettings');
    ...
    useEffect(() => {
        localStorage.setItem('EmployeeViolationsPrintSettings', JSON.stringify(printSettings));
    }, [printSettings]);
    ...
    const totalemployeesData = kpis?.total_violations || 0;
    const totalPending = kpis?.pending_violations || 0;
    const uniqueViolators = kpis?.unique_violators || 0;
    
    const chartData = safeChartData || [];
    const mostpendingDept = chartData.length > 0 && (chartData[0].pending > 0 || chartData[0].action_taken > 0) ? chartData[0].name : 'لا يوجد';

    let sortedEmployeesData = [...safeEmployees].sort((a, b) => {
        const aViolations = a.records.filter(r => r.status === 'قيد الإجراء' || r.status === 'تم اتخاذ إجراء').length;
        const bViolations = b.records.filter(r => r.status === 'قيد الإجراء' || r.status === 'تم اتخاذ إجراء').length;
        return bViolations - aViolations;
    });

    if (violatorsOnly) {
        sortedEmployeesData = sortedEmployeesData.filter(data => {
            const pendingCount = data.records.filter(r => r.status === 'قيد الإجراء').length;
            const actionTakenCount = data.records.filter(r => r.status === 'تم اتخاذ إجراء').length;
            return pendingCount >= 1 || actionTakenCount >= 3;
        });
    }

    const handleFilter = (e) => {
        if(e) e.preventDefault();
        router.get(route('hr.reports.employee-violations'), {
        ...
        try {
            ...
            const tableBody = [
                ...
                [
                    ...
                    { text: 'الإجراء المتخذ', style: 'tableHeader', alignment: 'center' }
                ]
            ];
            ...
            sortedEmployeesData.forEach(data => {
                data.records.forEach(record => {
                    tableBody.push([
                        ...
                        { text: record.action_taken ? record.action_taken : '-', alignment: 'center' }
                    ]);
                    count++;
                });
            });
            ...
            const docDefinition = {
                ...
                content: [
                    { text: printSettings.title || 'تقرير مخالفات الموظفين', style: 'header' },
                    {
                        columns: [
                            { text: `من: ${startDate || '-'}`, alignment: 'right' },
                            { text: `إلى: ${endDate || '-'}`, alignment: 'right' },
                            { text: `إجمالي المخالفات: ${totalemployeesData}`, alignment: 'right' },
                            { text: `إجمالي قيد الإجراء: ${totalPending}`, alignment: 'right' }
                        ],
                        margin: [0, 0, 0, 20]
                    },
                    {
            ...
            pdfMake.createPdf(docDefinition).download('employee_violations.pdf');
        } catch (error) {
                ...
                <div className="print:hidden relative overflow-hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        <div className="p-3.5 bg-white border border-primary-100 shadow-sm rounded-xl text-primary-600 flex-shrink-0 relative overflow-hidden group">
                            ...
                            <AlertTriangle size={28} strokeWidth={2} className="relative z-10" />
                        </div>
                        <div>
                            <h1 className="text-[22px] font-black text-slate-800 tracking-tight mb-1">
                                تقرير مخالفات الموظفين
                            </h1>
                            <p className="text-[13.5px] font-bold text-slate-500">عرض وطباعة سجلات المخالفات والإجراءات المتخذة للموظفين</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start relative">
                    ...
                    {printSettings.showKPIs && sortedEmployeesData.length > 0 && (
                        <div className="grid grid-cols-4 gap-3 mb-8">
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي المخالفات</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{totalemployeesData}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي الإجراءات</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{totalPending}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">الموظفين المخالفين</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{uniqueViolators}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                    ...
                                    <p className="text-sm font-black leading-tight text-slate-800 truncate" title={mostpendingDept}>{mostpendingDept}</p>
                                </div>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="h-[240px] w-full" dir="ltr">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                                        ...
                                        <Bar dataKey="pending" name="قيد الإجراء" stackId="a" fill="#ef4444" radius={[0, 0, 4, 4]} barSize={28} />
                                        <Bar dataKey="action_taken" name="تم اتخاذ إجراء" stackId="a" fill="#f59e0b" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                    ...
                    {sortedEmployeesData.length > 0 ? (
                            ...
                            <table className="w-full text-right border-collapse text-sm">
                                <thead className={`${printSettings.ecoMode ? 'bg-slate-100 text-slate-800 border-b-2 border-slate-800' : 'text-white'}`} style={!printSettings.ecoMode ? { backgroundColor: printSettings.brandColor } : {}}>
                                    <tr>
                                        ...
                                        <th className="py-3 px-4 font-bold border-l border-white/20">نوع المخالفة</th>
                                        <th className="py-3 px-4 font-bold text-center w-32 border-l border-white/20">مستوى التكرار</th>
                        ...
                        <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-100 border-dashed print:hidden">
                            ...
                            <h3 className="text-lg font-bold text-slate-800 mb-1">لا توجد سجلات مخالفات</h3>
                            <p className="text-slate-500">لم يتم العثور على أي مخالفات تطابق الفلاتر المحددة.</p>
                        </div>
                    )}
...
const EmployeeRowGroup = ({ employeeName, data, printSettings }) => {
    ...
    const pendingRecords = recordsArray.filter(r => r.status === 'قيد الإجراء');
    const actionTakenRecords = recordsArray.filter(r => r.status === 'تم اتخاذ إجراء');
    const pendingCount = pendingRecords.length;
    const actionTakenCount = actionTakenRecords.length;
    const isWarning = pendingCount >= 1 || actionTakenCount >= 3;
    ...
    return (
        <React.Fragment>
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className={`font-black text-lg ${isWarning ? 'text-red-900' : 'text-slate-800'}`}>{employeeName}</span>
                        ...
                        <div className="flex gap-2 items-center">
                            {pendingCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/10 print:bg-transparent print:ring-red-700">
                                    ...
                                    <span>قيد الإجراء: {pendingCount}</span>
                                </div>
                            )}
                            {actionTakenCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center sm:gap-1.5">
                                        <span>تم اتخاذ إجراء: {actionTakenCount}</span>
                                    </div>
                                </div>
                            )}
                            {pendingCount === 0 && actionTakenCount === 0 && (
            ...
            {recordsArray.map((violation, index) => (
                <tr 
                    key={violation.id} 
                ...
                >
                    ...
                    <td className="py-3 px-4 text-slate-700 font-bold">{violation.type_name}</td>
                    <td className="py-3 px-4 text-center">
                        <span className="font-semibold text-slate-600">{violation.repetition_level ? violation.repetition_level : '-'}</span>
                    </td>
                    <td className="py-3 px-4 text-center">
                        {violation.status === 'قيد الإجراء' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/10 print:bg-transparent print:ring-red-700"><XCircle size={12} className="print:hidden"/> {violation.status}</span>}
                        {violation.status === 'تم اتخاذ إجراء' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700"><CheckCircle size={12} className="print:hidden"/> {violation.status}</span>}
                    </td>
                </tr>
            ))}
        </React.Fragment>
