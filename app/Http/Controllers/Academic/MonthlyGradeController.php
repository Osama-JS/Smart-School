...
class MonthlyGradeController extends Controller implements \Illuminate\Routing\Controllers\HasMiddleware
{
    ...
    public function getMonthlyGradesReportFilterData(Request $request, Division $division, $subject_id, ResultPeriod $period)
    {
        $user = auth()->user();
        $isTeacher = $user && $user->role && $user->role->name === 'معلم';
        $isAdmin = $user && $user->role && in_array($user->role->name, ['مدير النظام', 'مدير الفرع']);
    ...
    {
        $data = $this->getMonthlyGradesReportFilterData($request, $division, $subject_id, $period);
        return Inertia::render('Teacher/MonthlyGrades/ReportView', $data);
    }

    public function downloadMonthlyGradesReportPdf(Request $request, Division $division, $subject_id, ResultPeriod $period)
    {
        $data = $this->getMonthlyGradesReportFilterData($request, $division, $subject_id, $period);
        ...
        $pdf = \Spatie\LaravelPdf\Facades\Pdf::view('pdf.teacher.monthly-grades-report', $data)
            ->format($paperSize)
        ...
        return $pdf->withBrowsershot(function ($browsershot) {
            ...
            ->download('monthly_grades_report.pdf');
    }
