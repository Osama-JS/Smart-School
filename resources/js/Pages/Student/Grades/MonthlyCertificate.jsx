...
export default function MonthlyCertificate({ 
    studentName, 
    studentId, 
    gradeName, 
    periodName, 
    ...
    return (
        <div className="min-h-screen bg-slate-100 print:bg-white flex justify-center py-8 print:py-0 font-sans" dir="rtl">
            <Head title={`شهادة ${studentName} - ${periodName}`} />
            ...
            <div className="w-full max-w-[210mm] bg-white shadow-2xl print:shadow-none print:w-full print:h-screen relative overflow-hidden flex flex-col aspect-[1/1.414] mx-4 sm:mx-auto border border-slate-200 print:border-none">
                ...
                <div className="relative z-10 flex flex-col h-full p-10 sm:p-14">
                    ...
                    <div className="text-center mb-12 relative">
                        <h1 className="text-4xl sm:text-5xl font-black text-slate-800 mb-4 tracking-tight">شهادة الدرجات الشهرية</h1>
                        <div className="inline-flex gap-2">
                            <div className="bg-slate-100 text-slate-600 px-4 py-1.5 rounded-full font-bold text-sm border border-slate-200 [color-adjust:exact] [-webkit-print-color-adjust:exact]">
                                {semesterName}
                            </div>
                            <div className="bg-primary-50 text-primary-700 px-4 py-1.5 rounded-full font-bold text-sm border border-primary-100 [color-adjust:exact] [-webkit-print-color-adjust:exact]">
                                {periodName}
                            </div>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-right border-collapse mb-8">
                            <thead>
                                <tr className="border-b-2 border-slate-200">
                                    ...
                                    <th className="py-3 px-4 text-sm font-black text-slate-500 text-center">الشفهي</th>
                                    <th className="py-3 px-4 text-sm font-black text-slate-500 text-center">الواجبات</th>
                                    <th className="py-3 px-4 text-sm font-black text-slate-500 text-center">الاختبار</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {results.map((res, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="py-4 px-4 text-center font-bold text-slate-600 font-mono">{res.scores?.oral_total ?? '-'}</td>
                                        <td className="py-4 px-4 text-center font-bold text-slate-600 font-mono">{res.scores?.homework_total ?? '-'}</td>
                                        <td className="py-4 px-4 text-center font-bold text-slate-600 font-mono">{res.scores?.monthly_exam ?? '-'}</td>
                                        <td className="py-4 px-4 text-center font-black text-primary-700 font-mono bg-slate-50/50 [color-adjust:exact] [-webkit-print-color-adjust:exact]">{res.scores?.grand_total ?? '-'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
