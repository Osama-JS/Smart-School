...
<head>
    <meta charset="UTF-8">
    <title>{{ $printSettings['title'] ?? 'تقارير محاضر الاجتماعات' }}</title>
...
<body class="p-4 sm:p-8">
            ...
            <div class="flex flex-col gap-0.5 z-10 w-[38%] pt-0.5">
                ...
                <div class="mt-3 pr-3 border-r-[3px] py-0.5 brand-border">
                    <h1 class="text-xl font-black text-slate-900 tracking-tight whitespace-nowrap leading-none">
                        {{ $printSettings['title'] ?? 'تقارير محاضر الاجتماعات' }}
                    </h1>
                </div>
            </div>
    ...
    <div class="grid grid-cols-3 gap-3 mb-8">
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">إجمالي الاجتماعات</p>
                <p class="text-lg font-black leading-none text-slate-800">{{ $stats['total'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">اجتماعات مجدولة</p>
                <p class="text-lg font-black leading-none text-amber-600">{{ $stats['scheduled'] ?? 0 }}</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-300 rounded-xl p-3 flex items-center gap-3 relative overflow-hidden">
            ...
            <div>
                <p class="text-slate-500 text-[11px] font-bold mb-0.5">اجتماعات مكتملة</p>
                <p class="text-lg font-black leading-none text-emerald-600">{{ $stats['completed'] ?? 0 }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <table class="w-full text-right border-collapse border border-slate-200 mt-2">
        <thead>
            <tr class="brand-bg">
                ...
                <th class="border border-slate-200 p-2 font-bold">الاجتماع</th>
                <th class="border border-slate-200 p-2 font-bold text-center">التاريخ والوقت</th>
                <th class="border border-slate-200 p-2 font-bold text-center">النوع</th>
                <th class="border border-slate-200 p-2 font-bold text-center">المنسق</th>
                <th class="border border-slate-200 p-2 font-bold text-center">عدد المدعوين</th>
        ...
        <tbody>
            ...
            @forelse($meetings as $meeting)
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="border border-slate-200 p-2 font-bold text-slate-800">{{ $meeting->title }}</td>
                    <td class="border border-slate-200 p-2 text-center font-bold text-slate-700">
                        <div>
                            @php
                                $dateObj = \Carbon\Carbon::parse($meeting->date);
                            @endphp
                            <span dir="ltr">{{ $dateObj->format('Y-m-d') }}</span>
                        </div>
                    ...
                    <td class="border border-slate-200 p-2 text-center">
                        @if($meeting->type === 'online')
                            <span class="text-blue-600 font-bold text-xs">عن بعد</span>
                        @else
                            <span class="text-emerald-600 font-bold text-xs">حضوري</span>
                        @endif
                    </td>
                    <td class="border border-slate-200 p-2 text-center font-semibold">{{ $meeting->supervisor ? $meeting->supervisor->name : '-' }}</td>
                    <td class="border border-slate-200 p-2 text-center text-slate-600 font-bold">
                        {{ $meeting->participants ? count($meeting->participants) : 0 }}
                    </td>
                    <td class="border border-slate-200 p-2 text-center">
                        @if($meeting->status == 'scheduled')
                            <span class="text-amber-600 font-bold text-xs">مجدول</span>
                        @elseif($meeting->status == 'completed')
                            <span class="text-emerald-600 font-bold text-xs">مكتمل</span>
                        @else
                            <span class="text-rose-600 font-bold text-xs">ملغي</span>
                        @endif
                    </td>
                </tr>
            @empty
