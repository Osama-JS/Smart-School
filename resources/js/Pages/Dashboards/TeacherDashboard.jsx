...
export default function TeacherDashboard({ auth, todayTimetable, attendanceStatus, upcomingMeetings, stats, leaderboard, quickTasks = [], latestNews }) {
    ...
    // 2. To-Do List State (from Backend)
    ...
    const addTask = () => {
        ...
        router.post(route('dashboard.quick-tasks.store'), { text: newTaskText }, {
            ...
            onSuccess: () => {
                ...
                setIsAddingTask(false);
                setIsProcessing(null);
                // The page will reload props with the new task
            },
            onError: () => setIsProcessing(null)
        });
    };
                                    ...
                                    <div 
                                        ...
                                        className={`group flex items-center justify-between gap-3 p-4 rounded-2xl cursor-pointer transition-all duration-300 border relative overflow-hidden ${
                                            task.completed 
                                                ...
                                                : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-emerald-200 dark:hover:border-emerald-800/50 hover:shadow-sm hover:-translate-y-0.5'
                                        }`}
                                    >
