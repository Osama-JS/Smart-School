...
export default function TeacherAbsences({ absences, kpis, departmentChartData, teachers, departments, filters }) {
    ...
    const [selectedTeacher, setSelectedTeacher] = useState(
        filters?.employee_id ? { value: filters.employee_id, label: safeTeachers.find(t => t.id == filters.employee_id)?.name } : null
    );
    const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);

    const statusOptions = [
        { value: 'absent', label: 'غياب بدون عذر' },
        { value: 'late', label: 'تأخير' },
        { value: 'excused', label: 'استئذان' },
        { value: 'leave', label: 'إجازة / مرضي' }
    ];
    ...
    const [printSettings, setPrintSettings] = useState(() => {
        const defaultSettings = {
            title: 'تقرير حضور وانصراف المعلمين',
            showKPIs: true,
            showDetails: true,
        ...
        try {
            const saved = localStorage.getItem('teacherAbsencesPrintSettings');
    ...
    useEffect(() => {
        localStorage.setItem('teacherAbsencesPrintSettings', JSON.stringify(printSettings));
    }, [printSettings]);
    ...
    const totalAbsences = kpis?.total_absent || 0;
    const totalLates = kpis?.total_late || 0;
    ...
    if (violatorsOnly) activeFilters.push({ id: 'violators', label: `المتجاوزين فقط` });
    ...
    const renderImpactBar = (absencesCount, latesCount) => {
        const total = absencesCount + latesCount;
        const maxSegments = 5;
        ...
        <AdminLayout activeMenu="التقارير">
            ...
            <div className="space-y-6">
                <div className="print:hidden relative overflow-hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        <div className="p-3.5 bg-white border border-primary-100 shadow-sm rounded-xl text-primary-600 flex-shrink-0 relative overflow-hidden group">
                            ...
                            <Calendar size={28} strokeWidth={2} className="relative z-10" />
                        </div>
                        <div>
                                ...
                                <div className="xl:col-span-4 flex flex-col xl:flex-row justify-between items-center gap-4 mt-2">
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className={`font-bold text-[15px] leading-none mb-1 ${violatorsOnly ? 'text-rose-700' : 'text-slate-700'}`}>عرض المتجاوزين فقط</div>
                                                <div className={`text-xs font-semibold ${violatorsOnly ? 'text-rose-500' : 'text-slate-500'}`}>من تجاوز 3 غيابات/تأخيرات</div>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start relative">
                    ...
                    {printSettings.showKPIs && sortedGroupedAbsences.length > 0 && (
                        <div className="grid grid-cols-4 gap-3 mb-8">
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي الغياب</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{totalAbsences}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي التأخير</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{totalLates}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-slate-500 text-xs font-bold mb-0.5">المعلمين المخالفين</p>
                                    <p className="text-lg font-black leading-none text-slate-800">{uniqueTeachers}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-slate-500 text-xs font-bold mb-0.5 truncate">القسم الأكثر مخالفة</p>
                                    <p className="text-sm font-black leading-tight text-slate-800 truncate" title={mostAbsentDept}>{mostAbsentDept}</p>
                                </div>
                            </div>
                        </div>
                    )}
                            ...
                            <div className="h-[240px] w-full" dir="ltr">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                                        ...
                                        <Bar dataKey="absent" name="غياب" stackId="a" fill="#ef4444" radius={[0, 0, 4, 4]} barSize={28} />
                                        <Bar dataKey="late" name="تأخير" stackId="a" fill="#f59e0b" radius={[0, 0, 0, 0]} />
                                        <Bar dataKey="excused" name="استئذان" stackId="a" fill="#3b82f6" radius={[0, 0, 0, 0]} />
                                        <Bar dataKey="leave" name="إجازة" stackId="a" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </div>
                    ...
                    {sortedGroupedAbsences.length > 0 ? (
                        <div className="overflow-x-auto mt-4 rounded-xl border border-slate-200 shadow-sm print:shadow-none print:border-none print:rounded-none bg-white">
                            <table className="w-full text-right border-collapse text-sm">
                                ...
                                <tbody>
                                    {sortedGroupedAbsences.map(([teacherName, data]) => (
                                        <TeacherRowGroup 
                                            key={teacherName} 
                        ...
                        <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-100 border-dashed print:hidden">
                            ...
                            <h3 className="text-lg font-bold text-slate-800 mb-1">لا توجد سجلات غياب</h3>
                            <p className="text-slate-500">لم يتم العثور على أي حالات غياب تطابق الفلاتر المحددة.</p>
                        </div>
                    )}
...
const TeacherRowGroup = ({ teacherName, data, printSettings }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const recordsArray = Array.isArray(data?.records) ? data.records : (data?.records ? Object.values(data.records) : []);
    const absentRecords = recordsArray.filter(r => r.status_code === 'absent');
    const lateRecords = recordsArray.filter(r => r.status_code === 'late');
    const teacherAbsencesCount = absentRecords.length;
    const teacherLatesCount = lateRecords.length;
    const isWarning = teacherAbsencesCount >= 3 || teacherLatesCount >= 3;
    const totalLateMinutes = lateRecords.reduce((sum, r) => sum + (r.late_minutes || 0), 0);

    return (
        <React.Fragment>
            <tr 
                className={`group cursor-pointer border-y ${isWarning ? 'bg-red-50/30 border-red-100 hover:bg-red-50/60' : 'bg-slate-50/50 border-slate-100 hover:bg-slate-50'} transition-all duration-300 break-inside-avoid print:break-inside-avoid`}
                onClick={() => setIsExpanded(!isExpanded)}
            >
                <td colSpan="4" className={`py-3.5 px-4 border-r-[3px] ${isWarning ? 'border-r-red-500' : ''}`} style={!isWarning ? { borderRightColor: printSettings.brandColor } : {}}>
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 ${isWarning ? 'bg-red-100 border-red-200 text-red-600' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
                                    <Users size={18} />
                                </div>
                                <div>
                                    <div className="flex items-center gap-2">
                                        <span className={`font-black text-lg ${isWarning ? 'text-red-900' : 'text-slate-800'}`}>{teacherName}</span>
                                        {isWarning && <AlertTriangle size={16} className="text-red-500 animate-pulse print:animate-none print:text-red-700 hidden print:block" />}
                                    </div>
                                    <div className={`text-xs font-medium mt-0.5 ${isWarning ? 'text-red-600/80' : 'text-slate-500'}`}>
                                        قسم {data.department}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex gap-2 items-center">
                            {teacherAbsencesCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/10 print:bg-transparent print:ring-red-700">
                                    <XCircle size={14} className="print:hidden text-red-500" />
                                    <span>غياب: {teacherAbsencesCount}</span>
                                </div>
                            )}
                            {teacherLatesCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center sm:gap-1.5">
                                        <span>تأخير: {teacherLatesCount}</span>
                                        {totalLateMinutes > 0 && (
                                            <span className="text-xs sm:border-r sm:border-amber-700/20 sm:pr-2 opacity-90">
                                                {totalLateMinutes} دقيقة
                                                {totalLateMinutes >= 180 && <span className="text-red-600 font-black mr-1 pr-1.5 border-r border-amber-700/20">(تجاوز الحد!)</span>}
                                            </span>
                                        )}
                                    </div>
                                </div>
                            )}
                            {teacherAbsencesCount === 0 && teacherLatesCount === 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 print:bg-transparent print:ring-emerald-700">
                                    ...
                                    <span>استئذانات: {recordsArray.length}</span>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {recordsArray.map((absence, index) => (
                <tr 
                    key={absence.id} 
                ...
                >
                    ...
                    <td className="py-3 px-4 text-slate-700 font-bold">{absence.day}</td>
                    <td className="py-3 px-4 text-center" dir="ltr">
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-50 border border-slate-100 text-slate-600 font-mono text-xs tracking-wider shadow-sm print:shadow-none print:bg-transparent print:border-none print:p-0">
                            ...
                            <span>{formatDateStr(absence.date)}</span>
                        </div>
                    </td>
                    <td className="py-3 px-4 text-center">
                        {absence.status_code === 'absent' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-red-50 text-red-700 ring-1 ring-inset ring-red-600/10 print:bg-transparent print:ring-red-700"><XCircle size={12} className="print:hidden"/> {absence.status}</span>}
                        {absence.status_code === 'late' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700"><Clock size={12} className="print:hidden"/> {absence.status} {absence.late_minutes ? `(${absence.late_minutes} د)` : ''}</span>}
                        {absence.status_code === 'excused' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20 print:bg-transparent print:ring-blue-700"><UserCheck size={12} className="print:hidden"/> {absence.status}</span>}
                        {absence.status_code === 'leave' && <span className="inline-flex items-center gap-1 justify-center px-2 py-1 text-xs font-bold rounded-md bg-teal-50 text-teal-700 ring-1 ring-inset ring-teal-600/20 print:bg-transparent print:ring-teal-700"><Calendar size={12} className="print:hidden"/> {absence.status}</span>}
                        {!['absent', 'late', 'excused', 'leave'].includes(absence.status_code) && <span>{absence.status}</span>}
                    </td>
                </tr>
            ))}
        </React.Fragment>
