...
export default function StudyPlansReport({ 
    auth, 
    teachers = [], 
    kpis = {}, 
    departmentChartData = [], 
    ...
    const statusOptions = [
        ...
        { value: 'missing', label: 'لم تُقدم (تقصير)' }
    ];
    ...
    const [printSettings, setPrintSettings] = useState(() => {
        const defaultSettings = {
            title: 'تقرير الخطط الدراسية وانضباط التسليم',
            showKPIs: true,
            showDetails: true,
        ...
        try {
            const saved = localStorage.getItem('studyPlansReportPrintSettings');
    ...
    useEffect(() => {
        localStorage.setItem('studyPlansReportPrintSettings', JSON.stringify(printSettings));
    }, [printSettings]);
    ...
    const totalPlans = kpis?.total_plans || 0;
    const totalApproved = kpis?.total_approved || 0;
    const totalPending = kpis?.total_pending || 0;
    const totalRejected = kpis?.total_rejected || 0;
    const totalDraft = kpis?.total_draft || 0;
    const totalMissing = kpis?.total_missing || 0;
    const approvalRate = kpis?.approval_rate || 0;

    const chartData = safeChartData || [];

    let sortedTeachers = [...safeTeachers].sort((a, b) => (b.rejected_plans + b.missing) - (a.rejected_plans + a.missing));

    if (violatorsOnly) {
        sortedTeachers = sortedTeachers.filter(t => t.rejected_plans > 0 || t.missing > 0);
    }

    const handleFilter = (e) => {
        if(e) e.preventDefault();
        router.get(route('hr.reports.study-plans'), {
    ...
    const handleExportCSV = () => {
        // Export CSV logic
        const csvRows = [];
        csvRows.push(['اسم المعلم', 'القسم', 'عنوان الخطة', 'المادة', 'الصف', 'الحالة', 'تاريخ التقديم', 'الملاحظات']);
        ...
        link.setAttribute("download", `study_plans_report_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    return (
        <AdminLayout user={auth?.user} activeMenu="التقارير">
            <Head title="تقارير الخطط الدراسية" />
                ...
                <div className="print:hidden relative overflow-hidden bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        <div className="p-3.5 bg-white border border-primary-100 shadow-sm rounded-xl text-primary-600 flex-shrink-0 relative overflow-hidden group">
                            ...
                            <Map size={28} strokeWidth={2} className="relative z-10" />
                        </div>
                        <div>
                            <h1 className="text-[22px] font-black text-slate-800 tracking-tight mb-1">
                                تقارير الخطط الدراسية
                            </h1>
                            <p className="text-[13.5px] font-bold text-slate-500">متابعة إعداد ونشر الخطط الدراسية الفصلية والشهريّة ونسب إنجاز المعلمين</p>
                        </div>
                    </div>
                            ...
                            <div className="overflow-hidden">
                                <form onSubmit={handleFilter} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                                    ...
                                    <div className="xl:col-span-2">
                                        <label className="block text-sm font-semibold text-slate-700 mb-1.5">حالة الخطة الدراسية</label>
                                        <Select
                                            isMulti
                                        ...
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <div className={`text-xs font-semibold ${violatorsOnly ? 'text-rose-500' : 'text-slate-500'}`}>من لديهم خطط مرفوضة أو عدم تسليم الخطة</div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="w-full max-w-[230px] bg-slate-50/50 print:bg-transparent border border-slate-200 print:border-slate-300 rounded-lg overflow-hidden shadow-sm print:shadow-none">
                                            ...
                                            <div className="p-3 flex flex-col gap-2.5">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span className="font-black text-slate-800 text-[11px]" dir="ltr">SP-{new Date().getFullYear().toString().substr(-2)}0{Math.floor(Math.random() * 900) + 100}</span>
                                                </div>
                                                ...
                                                <div className="flex justify-between items-center">
                                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">نسبة الاعتماد</span>
                                                    <span className="font-bold text-emerald-700 text-[11px] font-mono tracking-wide" dir="ltr">{approvalRate}%</span>
                                                </div>
                            ...
                            {printSettings.showKPIs && sortedTeachers.length > 0 && (
                                <div className="grid grid-cols-4 gap-3 mb-8">
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">إجمالي الخطط</p>
                                            <p className="text-lg font-black leading-none text-slate-800">{totalPlans}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">الخطط المعتمدة</p>
                                            <p className="text-lg font-black leading-none text-emerald-700">{totalApproved}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs font-bold mb-0.5">قيد المراجعة / مسودة</p>
                                            <p className="text-lg font-black leading-none text-amber-700">{totalPending + totalDraft}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white border border-slate-200 rounded-xl p-3 flex items-center gap-3 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden">
                                        ...
                                        <div className="overflow-hidden">
                                            <p className="text-slate-500 text-xs font-bold mb-0.5 truncate">مرفوضة / عدم تسليم</p>
                                            <p className="text-lg font-black leading-none text-rose-700">{totalRejected + totalMissing}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            {printSettings.showKPIs && chartData.length > 0 && (
                                <div className="mt-4 bg-white border border-slate-200 rounded-xl p-5 shadow-sm print:shadow-none print:border-slate-300 relative overflow-hidden mb-6">
                                    ...
                                    <h4 className="text-[15px] font-black text-slate-800 mb-5 flex items-center gap-2">
                                        إحصائيات انضباط الخطط الدراسية حسب الأقسام
                                    </h4>
                                    <div className="h-[240px] w-full" dir="ltr">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                                                ...
                                                <Bar dataKey="pending" name="قيد المراجعة" stackId="a" fill="#f59e0b" radius={[0, 0, 0, 0]} />
                                                <Bar dataKey="rejected" name="مرفوضة" stackId="a" fill="#ef4444" radius={[0, 0, 0, 0]} />
                                                <Bar dataKey="missing" name="لم تُقدم" stackId="a" fill="#94a3b8" radius={[4, 4, 0, 0]} />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>
                                </div>
                                    ...
                                    <table className="w-full text-right border-collapse text-sm">
                                        <thead className={`${printSettings.ecoMode ? 'bg-slate-100 text-slate-800 border-b-2 border-slate-800' : 'text-white'}`} style={!printSettings.ecoMode ? { backgroundColor: printSettings.brandColor } : {}}>
                                            <tr>
                                                ...
                                                <th className="py-3 px-4 font-bold border-l border-white/20">عنوان الخطة</th>
                                                <th className="py-3 px-4 font-bold border-l border-white/20">المادة والصف</th>
                                                <th className="py-3 px-4 font-bold text-center w-28 border-l border-white/20">الشهر / الفترة</th>
                                                <th className="py-3 px-4 font-bold text-center w-36 border-l border-white/20">تاريخ التقديم</th>
                                                <th className="py-3 px-4 font-bold text-center w-32 border-l border-white/20 last:rounded-tl-xl print:last:rounded-none">الحالة</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {sortedTeachers.map((teacher) => (
                                                <TeacherStudyPlanGroup 
                                                    key={teacher.id} 
                                                    teacher={teacher} 
...
const TeacherStudyPlanGroup = ({ teacher, printSettings }) => {
    const [isExpanded, setIsExpanded] = useState(true);
    const recordsArray = Array.isArray(teacher?.records) ? teacher.records : (teacher?.records ? Object.values(teacher.records) : []);
    
    const approvedCount = teacher.approved_plans || 0;
    const pendingCount = teacher.pending_plans || 0;
    const rejectedCount = teacher.rejected_plans || 0;
    const draftCount = teacher.draft_plans || 0;
    const missingCount = teacher.missing || 0;
    
    const isWarning = rejectedCount > 0 || missingCount > 0;
        ...
        <React.Fragment>
                ...
                <td colSpan="6" className={`py-3.5 px-4 border-r-[3px] ${isWarning ? 'border-r-amber-500' : ''}`} style={!isWarning ? { borderRightColor: printSettings.brandColor } : {}}>
                    <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <div className={`text-xs font-medium mt-0.5 ${isWarning ? 'text-amber-700/80' : 'text-slate-500'}`}>
                                        قسم {teacher.department} • إجمالي الخطط: {teacher.total_plans || recordsArray.length}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2 items-center">
                            ...
                            {missingCount > 0 && (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-bold bg-slate-100 text-slate-700 ring-1 ring-inset ring-slate-400/20 print:bg-transparent print:ring-slate-700">
                                    <AlertCircle size={14} className="print:hidden text-slate-500" />
                                    <span>لم تُقدم: {missingCount}</span>
                                </div>
                            )}
                        </div>
                    </div>
            ...
            {recordsArray.map((record, index) => (
                ...
                >
                    ...
                    <td className="py-3 px-4 text-slate-800 font-bold">{record.title}</td>
                    <td className="py-3 px-4 text-slate-700 font-semibold">
                        {record.subject_name} ({record.grade_name})
                    </td>
                    <td className="py-3 px-4 text-center font-mono text-xs font-bold text-slate-600">
                        {record.month}
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                        {record.status_code === 'approved' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-emerald-50 text-emerald-700 ring-1 ring-inset ring-emerald-600/20 print:bg-transparent print:ring-emerald-700">
                                <CheckCircle size={12} className="print:hidden"/> معتمدة
                            </span>
                        )}
                        {record.status_code === 'pending' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-600/20 print:bg-transparent print:ring-amber-700">
                                <Clock size={12} className="print:hidden"/> قيد المراجعة
                            </span>
                        )}
                        {record.status_code === 'rejected' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-rose-50 text-rose-700 ring-1 ring-inset ring-rose-600/20 print:bg-transparent print:ring-rose-700">
                                <XCircle size={12} className="print:hidden"/> مرفوضة
                            </span>
                        )}
                        {record.status_code === 'draft' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-blue-50 text-blue-700 ring-1 ring-inset ring-blue-600/20 print:bg-transparent print:ring-blue-700">
                                <FileText size={12} className="print:hidden"/> مسودة
                            </span>
                        )}
                        {record.status_code === 'missing' && (
                            <span className="inline-flex items-center gap-1 justify-center px-2.5 py-1 text-xs font-bold rounded-md bg-slate-100 text-slate-700 ring-1 ring-inset ring-slate-400/20 print:bg-transparent print:ring-slate-700">
                                <AlertCircle size={12} className="print:hidden"/> لم تُقدم
                            </span>
                        )}
                    </td>
                </tr>
