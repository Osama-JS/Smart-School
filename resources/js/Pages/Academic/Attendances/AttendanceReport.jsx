...
export default function AttendanceReport({ logs, filters, divisions }) {
    ...
    const applyFilters = () => {
        router.get(route('academic.attendances.report'), { date, search, status, division_id: divisionId }, { preserveState: true, replace: true });
    };
    ...
    const clearFilters = () => {
        ...
        setStatus('');
        setDivisionId('');
        router.get(route('academic.attendances.report'), { date: new Date().toISOString().split('T')[0] });
    };
                                            ...
                                            <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                                ...
                                                <td className="px-6 py-4">
                                                    {log.status === 'present' || log.status === 'حاضر' ? (
                                                        <span className="px-3 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 rounded-lg text-xs font-black print:bg-transparent print:border print:border-black print:text-black">حاضر</span>
                                                    ) : log.status === 'absent' || log.status === 'غائب' ? (
                                                        <span className="px-3 py-1 bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400 rounded-lg text-xs font-black print:bg-transparent print:border print:border-black print:text-black">غائب</span>
                                                    ) : log.status === 'late' || log.status === 'متأخر' ? (
                                                        <span className="px-3 py-1 bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400 rounded-lg text-xs font-black print:bg-transparent print:border print:border-black print:text-black">متأخر</span>
                                                    ) : log.status === 'excused' ? (
                                                        <span className="px-3 py-1 bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-400 rounded-lg text-xs font-black print:bg-transparent print:border print:border-black print:text-black">عذر / استئذان</span>
                                                    ) : (
                                                        <span className="px-3 py-1 bg-slate-100 text-slate-700 dark:bg-slate-500/20 dark:text-slate-400 rounded-lg text-xs font-black print:bg-transparent print:border print:border-black print:text-black">{log.status}</span>
                                                    )}
                                                </td>
                                            </tr>
                                        ))
