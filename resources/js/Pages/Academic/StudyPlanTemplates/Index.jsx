...
import { Plus, Edit2, Trash2, Save, X, PlusCircle, GripVertical, FileSignature, AlertCircle, Sparkles, LayoutTemplate, CheckCircle2, XCircle, CalendarDays, Check } from 'lucide-react';
    ...
    return (
        <AdminLayout activeMenu="قوالب الخطط الدراسية">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-right">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                            {templates.data && templates.data.length > 0 ? templates.data.map((item) => (
                ...
                {templates.links && <Pagination links={templates.links} />}
            </div>
                                        ...
                                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 relative z-10">
                                            {displayedMonths.map((m, idx) => (
                                                <button
                                                    key={idx}
                                                    type="button"
                                                    onClick={() => setData('month', m)}
                                                    className={`py-3 px-2 rounded-xl text-sm font-bold transition-all duration-300 flex flex-col items-center justify-center gap-1.5 border ${
                                                        data.month === m
                                                ...
                                                >
                                                    {data.month === m && <div className="absolute top-1.5 right-1.5 w-2 h-2 bg-white rounded-full animate-pulse" />}
                                                    {m}
                                                </button>
                                            ))}
                                        </div>
