...
class MeetingController extends Controller implements \Illuminate\Routing\Controllers\HasMiddleware
{
    ...
    public function singleReportPdf(Request $request, Meeting $meeting)
    {
        $meeting->load(['supervisor', 'participants.user']);
        ...
        $pdf = Pdf::view('pdf.hr.single-meeting-report', $data)
            ->format($paperSize)
            ->margins($margins[0], $margins[1], $margins[2] + 12, $margins[3])
            ->footerHtml($footerHtml);
        ...
        return $pdf->withBrowsershot(function ($browsershot) {
            ...
            ->download('meeting_report_' . $meeting->id . '.pdf');
    }
